...
const DOC = { id: "s1", user_id: "u1", graph_status: "ready", graph_error: null, layout: null }
const GRAPH = {
  topics: [
    {
      id: "t1",
...
describe("PATCH /api/graph/[syllabusId] (editable mind map)", () => {
  ...
  it("200 persists the edited graph and marks ready", async () => {
    ...
    const [sid, input] = vi.mocked(GraphRepository.replaceGraph).mock.calls[0]
    expect(sid).toBe("s1")
    expect((input as any).topics.map((t: any) => t.externalId).sort()).toEqual(["a", "b"])
    // prerequisites is now a separate top-level list (edge a→b, i.e. a before b)
    expect((input as any).prerequisites).toEqual([{ from: "a", to: "b" }])
    // DOC.layout is null (never generated by the new pipeline) — first edit defaults to radial
    expect((input as any).layout).toBe("radial")
