...
export function ChatWorkspaceProvider({ children }: { children: React.ReactNode }) {
  ...
  const addAttachment = useCallback(async (file: AttachedFile) => {
    ...
    if (file.file) {
      try {
        ...
        setAttachments((prev) => prev.map((f) => f.id === file.id ? { ...f, status: "ready", syllabus_id: data.syllabus_id } : f))
  ...
  const selectKnowledge = useCallback(async (upload: { id: string; original_filename: string }) => {
