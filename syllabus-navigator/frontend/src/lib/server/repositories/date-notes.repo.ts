...
export const DateNoteRepository = {
  ...
  async listByDate(userId: string, date: string): Promise<DateNote[]> {
    return (await sql`
      SELECT id, to_char(note_date, 'YYYY-MM-DD') AS note_date, body, created_at, updated_at
      FROM date_notes
  ...
  async create(userId: string, date: string, body: string): Promise<DateNote> {
    const rows = await sql`
      INSERT INTO date_notes (user_id, note_date, body)
      VALUES (${userId}::uuid, ${date}::date, ${body})
      RETURNING id, to_char(note_date, 'YYYY-MM-DD') AS note_date, body, created_at, updated_at
    `
    return rows[0] as DateNote
  },

  /** Edit a note the user owns. Returns null if it does not exist / isn't theirs. */
  async update(userId: string, id: string, body: string): Promise<DateNote | null> {
    const rows = await sql`
      UPDATE date_notes
