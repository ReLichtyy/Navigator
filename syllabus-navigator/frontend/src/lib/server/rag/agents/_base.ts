...
import { resolveAgentModel, type AgentRole } from "@/lib/llm/agent-models"
...
export async function runAgent<T>(c: AgentCall<T>): Promise<T | null> {
  const { provider, model, fallback } = resolveAgentModel(c.role)
  try {
    ...
    try {
      const fbProvider: LLMProvider = fallback.includes("/") ? "openrouter" : "openai"
      const raw = await callOnce(fbProvider, fallback, c)
      const parsed = c.schema.safeParse(JSON.parse(extractJson(raw)))
