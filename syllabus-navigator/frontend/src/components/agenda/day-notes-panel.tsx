...
interface Props {
  /** ISO yyyy-mm-dd of the open day. */
  date: string
  ...
  canEdit: boolean
  onClose: () => void
  /** Notifies the parent when the day's note count crosses 0 (to toggle markers). */
  onCountChange?: (date: string, hasNotes: boolean) => void
}

/**
 * Inline day panel: events + notes for one date, rendered as an in-place
 * expansion of the calendar (no drawer / separate layout). Notes CRUD scoped
 * to the authenticated user via /api/notes.
 */
export function DayNotesPanel({ date, dayEvents, canEdit, onClose, onCountChange }: Props) {
  ...
  // Load the day's notes whenever the open day changes.
  useEffect(() => {
    if (!canEdit) {
      setNotes([])
      return
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setNotes((prev) => {
        ...
        onCountChange?.(date, next.length > 0)
        return next
      })
      setDraft("")
    } catch {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setNotes(prev)
      onCountChange?.(date, prev.length > 0)
    }
  }

  return (
    <div className="animate-fade-in mt-3 rounded-2xl border border-accent/30 bg-accent/[0.04] p-4">
      <div className="mb-3 flex items-center justify-between gap-2 border-b border-border/60 pb-3">
        <div>
          <div className="text-sm font-bold capitalize text-foreground">{prettyDate(date)}</div>
        ...
        <Button size="icon-sm" variant="ghost" onClick={onClose} aria-label="Cerrar">
          <X className="h-4 w-4" />
        </Button>
      </div>
