...
| `src/components/` | Feature components: `GraphCanvas` (wraps `estudio/rich-mind-map-canvas` — xyflow was removed), `SelectionAsk`, `ClientProviders`; feature dirs `agenda/`, `estudio/`, `auth/`, `bienvenida/` |
...
| `courses` (GET/POST), `courses/[id]` (PATCH/DELETE) | Course Intelligence Layer: real course folders. DELETE cascades: the course's documents are deleted with it (2026-07-08; UI confirm warns first). PATCH = `{name?, term_start?}` — `term_start` anchors "Semana N" → real dates. `course-infer` also extracts it from the syllabus (stored on `course_suggestions`, applied on confirm only if the course has none — user-set value wins) |
| `graph/[syllabusId]` (GET/PATCH), `graph/[syllabusId]/reprocess` (POST) | Per-document graph: read; PATCH saves an edited graph (cycle-validated) — UI: structural editor in the canvas drawer, wired from the `/knowledge` preview (`GraphCanvas` + `src/lib/ui/graph-edit.ts`); re-enqueue ingest |
| `graph/course/[courseId]` (GET/PATCH), `graph/course/[courseId]/regenerate` (POST) | **Whole-course mind map** (2026-07-08, design Navigator.dc): one JSONB graph per course (`course_graphs` table, NOT `topics`), generated synchronously from the docs selected in the canvas' "Regenerar con IA" drawer (`fileIds` multi-select persisted as `source_doc_ids`, + `focusTopics`/`instructions` → `graph-gen` prompt). `/mapa` is course-level now (no doc selector; "sin curso" folders fall back to the per-doc graph); auto-generates on first visit with all ready docs. GET `graph_status:"none"` = never generated |
...
| `study/[syllabusId]`, `study/course/[courseId]` (GET) | Study set (bank-assembled; `?refresh&difficulty&topic&web`) per doc or whole course. Generated **on demand** when a mode opens (never on scope selection) |
| `study/[syllabusId]/status`, `study/course/[courseId]/status` (GET) | Light status (SQL-only, no LLM): bank counts + whether the default set is cached — the estudio menu renders instantly from this and only silently hydrates the set on a cache hit |
...
### Study area (`/estudio`) — current map and invariants

The study feature spans four layers; keep changes inside these boundaries:

| Layer | Main locations | Responsibility |
|---|---|---|
| Page orchestration | `app/estudio/page.tsx` | Auth gate, course/document scope, mode routing, focus/web options, cached-set hydration |
| Study UI | `src/components/estudio/` | Flashcards, staged quiz, review queue, exam, summary, mastery and generation progress |
| Client API | Study section in `src/lib/api.ts` | The only browser-to-server calls and the shared study DTOs |
| Server domain | `app/api/study/`, `services/study*.service.ts`, `repositories/study*.repo.ts`, `rag/orchestrator/`, `rag/agents/`, `rag/eval/` | Ownership/validation, set and bank assembly, adaptive planning, persistence, LLM generation |

Study-specific rules:

- Scope is a discriminated union. Server-backed adaptive features accept only one `doc` or one real `course`; client-only `docs` and `combo` scopes may combine flashcards/summary but cannot silently call quiz, review, exam, mastery or SRS endpoints.
- Selecting a scope must stay cheap: fetch status and warm the bank in the background; generate a study set only when a set-consuming mode opens or the user applies a focus.
- A default medium set may be cached. Topic-focused, non-medium, refreshed and web-augmented sets are fresh. Never persist web-augmented material as the canonical cache.
- Quiz questions move through three stores: `study_items` (bank), `quiz_seen` (do not repeat) and `quiz_review` (failed items). Preserve option shuffling whenever an item is served.
- Mastery and flashcard reviews are scope-based. Use `StudyScopeSchema`/ownership checks before writes; do not reintroduce document-only assumptions for real-course scope.
- Slow requests can finish after the user switches scope. Any new client-side loader must abort, version, or otherwise ignore stale responses before updating visible study state.
- Keep route handlers thin and validate query/body input with zod. Bank generation, grading and set assembly belong in services; SQL belongs in repositories.
- The large orchestration files are refactor candidates. Prefer extracting cohesive hooks/components/services over adding more state or domain branches to `app/estudio/page.tsx`, `quiz-view.tsx`, `exam-view.tsx`, `rich-mind-map-canvas.tsx` or `study.service.ts`.
...
- Tables: `users`, `user_preferences`, `syllabus_uploads`, `chunks`, `programs`, `courses`,
  ...
  `topic_mastery`, `user_courses`, `course_suggestions`, `course_study_sets`, `course_graphs`,
  `study_items` (item bank, embedding-deduped), `quiz_review`, `quiz_seen`, `exam_attempts`.
...
| `RAG_MAX_DISTANCE` | no | Cosine cutoff for retrieval relevance gate (default `0.75`) |
